...
private:
	...
	vec2 m_resizeSum;
public:
	cBackground(const char *sFileName, const vec2 &fSize, vec2 resize, vec2 resizePos, vec2 resizeSum);

	virtual void Render() override;

	inline const char* GetAudio() { return m_song; };
	inline void SetAudio(const char* sz_song) { m_song = sz_song; };
};
