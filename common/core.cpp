...
//-----------------------------------------------------------------------------
void CORE_RenderCenteredRotatedSprite(vec2 pos, vec2 size, float angle, GLuint texid, rgba_t color, float u0, float v0, float u1, float v1)
{
#if 0 // MANUAL
  // Rotated axes
  ...
  glBegin( GL_QUADS );
  glColor4f(GETR(color)/255.f, GETG(color)/255.f, GETB(color)/255.f, GETA(color)/255.f);
  glTexCoord2d(u0, 0.0); glVertex2f(-0.5f, -0.5f);
  glTexCoord2d(v0, 0.0); glVertex2f(+0.5f, -0.5f);
  glTexCoord2d(v1, 1.0); glVertex2f(+0.5f, +0.5f);
  glTexCoord2d(u1, 1.0); glVertex2f(-0.5f, +0.5f);
  glEnd();
